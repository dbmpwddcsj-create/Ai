...
# ============================================================
# LLM (OpenRouter — бесплатные модели: DeepSeek, Qwen и др.)
# ============================================================
#
# OPENROUTER_API_KEY нужно получить бесплатно на https://openrouter.ai
# (регистрация без карты). Сами модели с суффиксом ":free" бесплатны,
# но список "живых" бесплатных моделей на OpenRouter периодически
# меняется — старые слаги отключают, появляются новые. Поэтому здесь
# задан ФОЛБЭК-СПИСОК: пробуем модели по порядку, при ошибке/404
# переходим к следующей. Если ключ не задан вообще — приложение
# просто работает в старом режиме (экстрактивные ответы без LLM).
#
...
# ============================================================
# LLM ANSWER SYNTHESIS (OpenRouter — free models, with fallback)
# ============================================================
...
def call_llm(system_prompt, user_prompt):
    """
    Пробует модели из LLM_MODELS по порядку. Возвращает текст ответа
    или None, если ни одна модель не сработала (нет ключа, все модели
    недоступны/устарели, сетевая ошибка и т.д.) — в этом случае вызывающий
    код должен откатиться на экстрактивный режим.
    """
    ...
    for model in LLM_MODELS:
        ...
        try:
            ...
            print("LLM HTTP:", response.status_code, "model=", model)

        except Exception as e:
            print("LLM REQUEST ERROR:", repr(e), "model=", model)
            continue

        if response.status_code == 404:
            # Модель устарела / слаг больше не существует — пробуем следующую.
            print("LLM MODEL GONE (404):", model)
            continue

        if response.status_code == 429:
            print("LLM RATE LIMITED, trying next model:", model)
            continue

        if response.status_code >= 400:
            print("LLM BAD STATUS:", response.status_code, response.text[:300])
            continue
...
def llm_answer_from_web(query, web_results):
    """
...
def generate_response(query, memory, local_results, web_results):
    ...
    if best_item and best_score >= 0.18:
        ...
        if llm_available():
            llm_answer = llm_answer_from_local(query, knowledge_answer, web_results)
            if llm_answer:
                return llm_answer
    ...
    if web_results:
        if llm_available():
            llm_answer = llm_answer_from_web(query, web_results)
            if llm_answer:
                return llm_answer
...
async def startup():
    ...
    print("=" * 60)
    print("Knowledge:", len(knowledge_cache))
    print("Neural brain:", brain.ready)
    print("Search engines:", [name for name, _ in SEARCH_ENGINES])
    print("SearXNG instances:", len(SEARXNG_INSTANCES))
    print("=" * 60)
    print("")
