...
# Обычный поиск Яндекса. API НЕ используется.
YANDEX_SEARCH_URLS = [
...
def unwrap_yandex_url(url):

    """
    Иногда Яндекс отдаёт ссылки через redirect/r=<url>.
    Пытаемся достать реальный адрес.
    """
...
def parse_yandex_results(
    ...
    # --------------------------------------------------------
    # Вариант 2. ищем заголовочные ссылки
    # --------------------------------------------------------

    if not results:

        for link_node in soup.find_all(
            "a"
        ):
            ...
            # Слишком короткие технические ссылки
            if title.lower() in {
                "войти",
                "картинки",
                "видео",
...
def yandex_search(
    ...
    """
    Поиск через ОБЫЧНУЮ страницу Яндекса.
    Yandex API не используется.
    """
...
def collect_web_information(
    ...
    if not search_results:
        ...
        print(
            "WEB INFORMATION: no search results"
        )
...
def fallback_web_answer(
    ...
    """
    Берём лучшие предложения из веб-поиска.
    Если страницы недоступны, используем сниппеты.
    """
    ...
    if not sentences:
        ...
        # Иногда найденный текст не содержит
        # точных совпадений после очистки.
        # Тогда берём первые осмысленные предложения.
...
def generate_response(
    ...
    return (
        "Я не смог получить достаточно "
        "подходящей информации из поиска.\n\n"
        "Попробуй сформулировать вопрос "
        "немного подробнее."
    )
...
async function sendMessage() {
    ...
    addMessage(
        "ai",
        "🌐 Ищу информацию в Яндексе и анализирую найденные источники..."
    );
