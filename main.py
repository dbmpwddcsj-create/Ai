...
# Ограничиваем длину входящих запросов
...
def supabase_request(
    ...
    ВАЖНО:
    SUPABASE_SECRET_KEY используется только на сервере Render.
    Никогда не помещаем его в HTML/JS.
    """
...
class NeuralBrain:
    ...
    """
    Небольшое собственное нейронное ядро.
    ...
    Это обучаемая нейросеть, написанная здесь без
    использования внешнего AI API.
    """
...
DEFAULT_KNOWLEDGE = [

    {
        ...
        "answer": """
Если кожа быстро становится жирной, не стоит пытаться постоянно
...
Если одновременно есть выраженное болезненное акне, лучше обратиться
к дерматологу.
""",
        "tags": [
            "жирная кожа",
...
Если акне тяжёлое, болезненное или оставляет рубцы, стоит обратиться
к дерматологу.
""",
        "tags": [
            "прыщи",
...
Лучше постепенно улучшать несколько направлений, чем искать одно
чудо-средство.
""",
        "tags": [
            "внешность",
...

    {
        ...
        "answer": """
Для внешнего вида обычно важнее сбалансированный рацион, чем
экстремальная диета.
...
Главное — стабильность режима, а не попытка компенсировать всё одной
долгой ночью.
""",
        "tags": [
            "сон",
...
def yandex_search(
    ...
    """
    Поиск через страницу Яндекса.

    Важно:
    это не Yandex AI и не Алиса.
    Сервер получает поисковую выдачу Яндекса.
    """
    ...
    # Если разметка Яндекса изменилась,
    # пробуем найти ссылки через все <a>.
    if not results:
...
def generate_response(
    ...
    # --------------------------------------------------------
    # 1. Если есть хорошее собственное знание
    # --------------------------------------------------------
    ...
    if (
        ...
        # Дополняем свежими данными из поиска,
    ...
    # --------------------------------------------------------
    # 2. Если локальная база слабая,
    #    пытаемся сформировать ответ из веб-данных.
    # --------------------------------------------------------
    ...
    if web_text:
        ...
        if sentences:
            ...
            answer += (
                "\n"
                "⚠️ Информация из интернета "
                "может быть разного качества, "
                "поэтому для важных вопросов "
                "лучше проверять первоисточники."
            )
    ...
    return (
        "Я пока не смог найти достаточно "
        "подходящей информации для уверенного ответа.\n\n"
        "Попробуй сформулировать вопрос подробнее."
    )
...
Каждый твой вопрос я дополнительно
проверяю через интернет.
...
async function sendMessage() {
    ...
    addMessage(
        "ai",
        "🌐 Ищу информацию в интернете и анализирую её..."
    );
        ...
        if (statsResponse.ok) {
            ...
            document.getElementById(
                ...
                <p>
                    🧠 Модель:
                    <strong>
                        ${stats.brain_ready ? "готова" : "не готова"}
                    </strong>
                </p>
...
async def admin_web_sources(
    ...
    rows = supabase_request(
        ...
        params={
            "select":
                "id,query,title,url,snippet,source,created_at",
