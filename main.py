...
#   3. Bing HTML (последний резерв)
#
# Если все три уровня не дали результатов — возвращаем пустой список,
...
# ============================================================
# SEARCH ENGINE 3: BING HTML (last-resort fallback)
# ============================================================

def bing_html_search(query, limit=MAX_SEARCH_RESULTS):
    query = query.strip()
    if not query:
        return []
    ...
    print("") 
    print("------------------------------------------") 
    print("BING QUERY:", query) 
 
    try: 
        response = requests.get( 
            url, 
            params={"q": query, "setlang": "ru", "mkt": "ru-RU"}, 
            headers=BROWSER_HEADERS, 
            timeout=SEARXNG_TIMEOUT, 
        ) 
 
        print("BING HTTP:", response.status_code) 
 
    except Exception as e: 
        print("BING REQUEST ERROR:", repr(e)) 
        return [] 
 
    if response.status_code >= 400: 
        print("BING BAD STATUS:", response.status_code) 
        return [] 
 
    try: 
        soup = BeautifulSoup(response.text, "html.parser") 
    except Exception as e: 
        print("BING PARSE ERROR:", repr(e)) 
        return [] 
    ...
    for i, r in enumerate(results, start=1): 
        print(f"BING RESULT {i}:", r.get("title", "")[:120], r.get("url", "")) 
    print("------------------------------------------") 
...
def fallback_web_answer(query, web_results): 
    web_text = build_web_context(web_results) 
    if not web_text: 
        return "" 
