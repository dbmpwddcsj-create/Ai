...
# ниже может их переопределить через админ-панель.
_DEFAULT_SETTINGS = {
...
SETTINGS_KEYS = ("openrouter_api_key", "deepseek_api_key", "qwen_api_key")
...
def llm_available():
    return any(get_setting(k) for k in SETTINGS_KEYS)
...
def call_llm(system_prompt, user_prompt):
    """
    Пробует провайдеров по порядку: OpenRouter (бесплатные модели) ->
    DeepSeek напрямую -> Qwen напрямую. Возвращает None, если ни один
    не сработал (нет ключей, все недоступны, сетевые ошибки) — тогда
    вызывающий код откатывается на экстрактивный режим без LLM.
    """
    ...
    for provider_fn in (_try_openrouter, _try_deepseek_direct, _try_qwen_direct):
...
async function saveSettings() {
    ...
    if (Object.keys(body).length === 0) {
        status.textContent = "Введи хотя бы один ключ перед сохранением.";
        return;
    }
    ...
    try {
        ...
        try { data = await response.json(); } catch { data = { detail: "Некорректный ответ сервера." }; }
        if (!response.ok) { status.textContent = data.detail || "Ошибка."; return; }
        status.textContent = "✅ Ключи сохранены.";
